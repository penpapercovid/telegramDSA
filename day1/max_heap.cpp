...
void maxHeapify(std::vector<int> &v, int i, int n) {
    ...
    while (2*i <= n) {
        j = 2*i;
        if (j < n && v[j] < v[j+1]) { //checking if right child is greater than left child if that is the case we can increase j to point to right child and then compare it with root node 
            ++j;
        }
        if (v[i] < v[j]) { // when right child is greater than root node we need to swap the nodes.
            std::swap(v[i], v[j]);
        } else {
            break;
        }
...
void maxHeapify(std::vector<int> &v, int i, int n) {
    int j = 0; // index for children of current index i
    while (2*i <= n) { // left child must be less than n
        j = 2*i;
        if (j < n && v[j-1] < v[j]) { //checking if right child is greater than left child if that is the case we can increase j to point to right child and then compare it with root node
            ++j;
        }
        if (v[i-1] < v[j-1]) { // when right child is greater than root node we need to swap the nodes.
...
int main() {
    ...
    minHeap(v1);
    traverse(v1);
    std::vector<int> v2 = {3,4,1,45,32,6,9,22};
    maxHeap(v2);
    traverse(v2);
